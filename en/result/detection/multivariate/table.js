...
const LeaderboardApp = {
    ...
    config: {
        // !! 重要：更新 API URL
        API_URL: 'https://www.opents.xyz/outlier/multi/query',
    ...
    data.forEach(item => {
      item.method = item.method.replaceAll(' (label)','').replaceAll(' (score)','').replaceAll(' (full)','')
    ...
    data.forEach(item => {
      ...
      metric = item.metric
      horizon=1
      method = method.replaceAll(' (label)','').replaceAll(' (score)','').replaceAll(' (full)','')
      method = method
