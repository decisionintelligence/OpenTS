...
function exportToCsv() {
    if (gridApi) {
        gridApi.exportDataAsCsv({
          fileName: 'foundation_forecasting.csv'
        });
    } else {
...
const LeaderboardApp = {
  // --- 1. CONFIGURATION & CONSTANTS (内容不变) ---
    config: {
        ...
        MODEL_TYPES_LIST: {"TS-Pretrain":["Chronos","MOIRAI","Moment","ROSE",'TimesFM',"Timer","TTM","UniTS","LightGTS","Sundial(128m)","Toto2.0(313m)"], "LLM-Based":["CALF","GPT4TS","LLMMixer","S2IPLLM","TimeLLM","UniTime"], "Specific":["PatchTST","Dlinear","FedFormer","FITS","TimeMixer","TimesNet","iTransformer"]},
        ...
        METRICS_ORDER:[],
        DATASETS_ORDER:[],
        METHODS_ORDER:Object.values({"TS-Pretrain":["Chronos","MOIRAI","Moment","ROSE",'TimesFM',"Timer","TTM","UniTS"], "LLM-Based":["CALF","GPT4TS","LLMMixer","S2IPLLM","TimeLLM","UniTime"], "Specific":["PatchTST","Dlinear","FedFormer","FITS","TimeMixer","TimesNet","iTransformer"]}).flat().sort(), 
    },
...
return pivotedData;
    ...
    _getSelections() {
        ...
        var settings = getCheckedValues('.checkbox-Settings', cb => cb.value).filter(Boolean).map(e => e.replace(/-/g, '_'));
        var methods = [...getCheckedValues('.checkbox-TS-Pretrain', cb => cb.value), ...getCheckedValues('.checkbox-LLM-Based', cb => cb.value), ...getCheckedValues('.checkbox-Specific', cb => cb.value)].filter(Boolean).map(e => e.replace(/-/g, '_'))
    ...
    _createCategoryElement(category) {
        ...
        if (categoryName=='TS-Pretrain' || categoryName=='LLM-Based')
        {
