...
const LeaderboardApp = {
  ...
  _renderTable(results, isRanked) {
    ...
    this.elements.tableBody.innerHTML = finalResults.map((result, index) => {
      let partern = result.model
      const info = this.config.MODELS_INFO[partern] || {};

      return `<tr><td>${isRanked ? index + 1 : ''}</td><td>${result.model.replace("_shot","")}</td><td>${result.rank4}</td><td>${result.rank1}</td><td>${result.rank2}</td><td>${result.rank3}</td><td><a href="${info['paper-url'] || '#'}" target="_blank">paper</a></td><td>${info.publication || 'N/A'} ${info.year}</td></tr>`;
    }).join('');
    this.sendHeight();
  },
  _renderParameters(p){
