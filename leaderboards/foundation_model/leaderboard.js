...
const LeaderboardApp = {
  ...
  config: {
    API_URL: 'https://www.opents.top/foundts/multi/rank',
    MODELS_INFO: { 
      ...
      "GPT4TS": {"paper-url": "https://openreview.net/forum?id=gMS6FVZvmF", "publication": "NeurIPS", "bib": "https://dblp.org/rec/conf/iclr/NieNSK23.html?view=bibtex", "year": "2023", "parameters": "65380704"}, 
      ...
      "TTMs": {"paper-url": "https://arxiv.org/pdf/2401.03955", "publication": "NeurIPS", "bib": "https://dblp.uni-trier.de/rec/conf/nips/EkambaramJ0MNGR24.html?view=bibtex", "year": "2024", "parameters": "805287"}, 
      "UniTS": {"paper-url": "https://arxiv.org/pdf/2403.00131", "publication": "NeurIPS", "bib": "https://dblp.uni-trier.de/rec/conf/nips/GaoKQHTZ24.html?view=bibtex", "year": "2024", "parameters": "3459007"}, 
  ...
  _renderTable(results, isRanked) {
    ...
    this.elements.tableBody.innerHTML = finalResults.map((result, index) => {
      ...
      return `<tr><td>${isRanked ? index + 1 : ''}</td><td>${result.model.replace("_shot","")}</td><td>${this._renderParameters(info['parameters'])}</td><td>${result.rank4}</td><td>${result.rank1}</td><td>${result.rank2}</td><td>${result.rank3}</td><td><a href="${info['paper-url'] || '#'}" target="_blank">paper</a></td><td>${info.publication || 'N/A'} [<a href="${info.bib || '#'}" target="_blank">bib</a>]</td><td>${info.year || 'N/A'}</td></tr>`;
    }).join('');
    this.sendHeight();
  },
  _renderParameters(p){
